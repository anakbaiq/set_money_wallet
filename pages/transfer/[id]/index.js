...
const amountTransfer = () => {
  ...
  useEffect(() => {
    if(id){
      getRecipient(token, id).then((res) => {
        setProfile(res);
      });
    }
  }, [token, id]);
  const getRecipient = async (token, id) => {
...
const wallet = Number(user.balance).toLocaleString("id-ID");
  return (
    <>
      <Header />
            ...
            <form onSubmit={transferFilled} className="mb-[30px]">
              ...
              <div className="py-[30px] flex flex-col gap-5 ajutify-center items-center px-[5%]">
                <input
                  ...
                  required
                />
                <p className="font-bold text-[20px]">{`Rp.${wallet}`}</p>
